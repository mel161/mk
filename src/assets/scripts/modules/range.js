...
const RangeSlider = function(className) {
  ...
  const handlerTo = slider.querySelector(".range__handler--from");
  ...
  if (inputFrom.getAttribute("step") || inputTo.getAttribute("step")) {
    step = Math.abs(parseFloat(inputFrom.getAttribute("step") || inputTo.getAttribute("step")));
  }
  ...
  self.reset = function() {
    ...
    handlerTo.style.left = slider.offsetWidth - handlerFrom.offsetWidth + "px";
    rangeLine.style.marginLeft = "0px";
    rangeLine.style.width = slider.offsetWidth - handlerFrom.offsetWidth + "px";
    startX = 0;
    x = 0;
  };
  ...
  self.setMinValue = function(minValue) {
    ...
    handlerFrom.style.left =
      Math.ceil(
        ratio * (slider.offsetWidth - (handlerFrom.offsetWidth + normalizeFact))
      ) + "px";
  ...
  var maxX = slider.offsetWidth - handlerTo.offsetWidth;
  ...
  function onMove(event) {
    ...
    if (slider.getAttribute("on-change")) {
      ...
      fn(
        slider.getAttribute("data-min-value"),
        slider.getAttribute("data-max-value")
      );
    }
